...
void on_midi(void* p, const uint8_t* msg, int len, int /*source*/) {
    ...
    if (hi == 0x90 && len >= 3) {
        ...
        // Send list [pitch velocity] to [r notes] (Organelle convention).
        t_atom notes_av[2];
        SETFLOAT(&notes_av[0], static_cast<float>(pitch));
        SETFLOAT(&notes_av[1], static_cast<float>(msg[2]));
        libpd_list("notes", 2, notes_av);
...
void set_param(void* p, const char* key, const char* val) {
    ...
    // Knob values arrive 0-1023 (JS integrates deltas). Follow Organelle's
    // mother.pd convention: publish normalised (0-1) to [r knobN] and raw
    // (0-1023) to [r knobNRaw]. Many C&G patches read normalised; our
    // hand-written patches and some others use raw.
    auto publish_knob = [&](int n, float raw){
        char base[8], raw_recv[16];
        std::snprintf(base, sizeof(base), "knob%d", n);
        std::snprintf(raw_recv, sizeof(raw_recv), "knob%dRaw", n);
        send_float(base, raw / 1023.0f);
        send_float(raw_recv, raw);
    };
    if (std::strcmp(key, "knob1") == 0) { publish_knob(1, fv); return; }
    if (std::strcmp(key, "knob2") == 0) { publish_knob(2, fv); return; }
